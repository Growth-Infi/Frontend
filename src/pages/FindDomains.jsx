...
export default function FindDomains() {
    ...
    const { csvFile, columns, rows, companyCol, contextCol, results, loading, error } = data
    ...
    function handleCSV(e) {
        ...
        reader.onload = (ev) => {
            ...
            update({ csvFile: file, columns: cols, rows: data, delimiter, companyCol: cols[0] || "", contextCol: "" })
        }
    ...
    async function handleFind() {
        ...
        update({ loading: true, error: "", results: null })
        ...
        try {
            ...
            const merged = rows.map((r, i) => ({
                ...r,
                domain: data.domains[i] || "Not Found",
            }))
            update({ results: { merged, stoppedEarly: data.stoppedEarly, failCount: data.failCount }, loading: false })
        } catch (err) {
    ...
    return (
        <div className="flex flex-col gap-6">
            <div>
                ...
                <p className="text-sm text-stone-500 mt-1">Resolve company names to website domains via search.</p>
            </div>
                ...
                {columns.length > 0 && (
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-medium text-stone-500">Company name column</label>
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-medium text-stone-500">Context column (optional)</label>
                            <select value={contextCol} onChange={e => update({ contextCol: e.target.value })}
                                className="text-sm border border-stone-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 bg-white">
                            ...
                            <tbody>
                                {results.merged.map((r, i) => (
                                    <tr key={i} className="border-b border-stone-100 last:border-0 hover:bg-stone-50 transition-colors">
                                        {columns.map(c => <td key={c} className="px-5 py-3.5 text-stone-700">{r[c]}</td>)}
